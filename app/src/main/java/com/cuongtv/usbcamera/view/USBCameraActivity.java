...
import com.cuongtv.usbcamera.tflite.Classifier;
...
import com.cuongtv.usbcamera.application.MyApplication;
import com.cuongtv.usbcamera.tracking.MultiBoxTracker;
import com.jiangdg.usbcamera.utils.FileUtils;
import com.cuongtv.usbcamera.utils.ImageUtils;
...
public class USBCameraActivity extends AppCompatActivity implements CameraDialog.CameraDialogParent, CameraViewInterface.Callback {
    ...
    private static final int cropSize = 256;
    private static final int cropSize_rescale = 256;
...
//        canvas.drawBitmap(rgbFrameBitmap, frameToCropTransform, null);
        ...
        runInBackground(new Runnable() {
            @Override
            public void run() {
                Bitmap mCropBitmap = scaleCenterCrop(rgbFrameBitmap, previewHeight, previewHeight);
                Bitmap resizedBitmap = Bitmap.createScaledBitmap(mCropBitmap, cropSize, cropSize, false);
                Bitmap croppedCircleBitmap = cropCircle2(resizedBitmap);
                float[][][] img = normalizeImage(croppedCircleBitmap);
                ...
                RectF rectf = new RectF(5, 100, 252, 234);
                ...
                if (outputs[0][0] > outputs[0][1]) {
                    imageQueue.addImageResult(0);
                } else {
                    imageQueue.addImageResult(1);
                }
                int historyRes = imageQueue.getFinalResult();
                if (historyRes == 0) {
                    Log.d("Cuongcuong", "currTimestamp: " + currTimestamp + " abnormal");
                    Log.d("Cuongcuong", outputs[0][0] + " " + outputs[0][1]);
                    mobject = new Recognition("0", "abnormal", 1F, rectf);
                    paint.setColor(Color.RED);
                    Log.d("Cuongcuong", "Mau set: " + paint.getColor() + " mau do " + Color.RED);
                } else {
                    Log.d("Cuongcuong", "currTimestamp: " + currTimestamp + " normal");
                    Log.d("Cuongcuong", outputs[0][0] + " " + outputs[0][1]);
                    mobject = new Recognition("1", "normal", 1F, rectf);
                    paint.setColor(Color.GREEN);
                    Log.d("Cuongcuong", "Mau set: " + paint.getColor() + " mau xanh la cay " + Color.GREEN);
                }
                results.add(mobject);
                ...
                for (final Recognition result : results) {
                    ...
                    if (location != null && result.getConfidence() >= minimumConfidence) {
                        Log.d("Cuongcuong", "Mau gi day: " + paint.getColor() + "");
                ...
                tracker.trackResults(mappedRecognitions, currTimestamp, paint.getColor());
...
//                break;
    ...
    public static Bitmap centerCrop(Bitmap source, int targetWidth, int targetHeight) {
        ...
        // Tính toán độ lệch tọa độ để crop ảnh theo chiều ngang và dọc
        int xOffset = (sourceWidth - targetWidth) / 2;
        int yOffset = (sourceHeight - targetHeight) / 2;

        // Tạo một Bitmap mới với kích thước đích
        Bitmap result = Bitmap.createBitmap(targetWidth, targetHeight, source.getConfig());

        // Copy phần center crop vào Bitmap mới
        for (int x = 0; x < targetWidth; x++) {
            for (int y = 0; y < targetHeight; y++) {
                int sourceX = x + xOffset;
                int sourceY = y + yOffset;

                // Đảm bảo không truy cập ngoài kích thước ảnh gốc
                sourceX = Math.min(Math.max(sourceX, 0), sourceWidth - 1);
                sourceY = Math.min(Math.max(sourceY, 0), sourceHeight - 1);

                // Lấy màu từ ảnh gốc và gán vào ảnh mới
    ...
    // Hàm cắt hình tròn từ bitmap gốc
    private Bitmap getCircularBitmap(Bitmap bitmap) {
        ...
        // Vẽ hình tròn đen có đường kính 256 trên canvas
        canvas.drawCircle(diameter / 2f, diameter / 2f, diameter / 2f, paint);

        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));

        // Vẽ bitmap gốc lên canvas với hiệu ứng cắt theo hình tròn
        canvas.drawBitmap(bitmap, 0, 0, paint);

        return circularBitmap;
    }

    public Bitmap cropCircle2(Bitmap source) {
        int targetSize = Math.min(source.getWidth(), source.getHeight());
        // Tạo bitmap màu đen với kích thước 256x256
        Bitmap blackBitmap = Bitmap.createBitmap(targetSize, targetSize, Bitmap.Config.ARGB_8888);
        blackBitmap.eraseColor(Color.BLACK);

        // Cắt hình tròn từ bitmap gốc
        Bitmap circularRegion = getCircularBitmap(source);

        // Dán hình tròn đã cắt vào bitmap màu đen
        Canvas canvas = new Canvas(blackBitmap);
        canvas.drawBitmap(circularRegion, 0, 0, null);
